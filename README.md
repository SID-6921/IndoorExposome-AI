...
Status: **Step 1 complete**, **pre-modeling confirmation complete** ([STEP1_5_PRE_MODELING_CONFIRMATION.md](STEP1_5_PRE_MODELING_CONFIRMATION.md)), **Step 2 preliminary baseline modeling complete** ([STEP2_PRELIMINARY_BASELINE_RESULTS.md](STEP2_PRELIMINARY_BASELINE_RESULTS.md)), **Step 2B event-level/calibration analysis complete** ([STEP2B_EVENT_LEVEL_CALIBRATION_ANALYSIS.md](STEP2B_EVENT_LEVEL_CALIBRATION_ANALYSIS.md)), **primary model frozen** ([FROZEN_CONFIG.md](FROZEN_CONFIG.md)), **10-home LBNL holdout evaluated once** ([STEP2C_HOLDOUT_RESULTS.md](STEP2C_HOLDOUT_RESULTS.md)), **100-Air-Purifiers external validation + calibration correction + sensitivity analysis complete** ([ROUND2_EXTERNAL_VALIDATION_AND_SENSITIVITY.md](ROUND2_EXTERNAL_VALIDATION_AND_SENSITIVITY.md), portable model frozen separately: [FROZEN_CONFIG_PORTABLE.md](FROZEN_CONFIG_PORTABLE.md)), **episode-definition divergence mechanism traced** ([DEFINITION_DIVERGENCE_ANALYSIS.md](DEFINITION_DIVERGENCE_ANALYSIS.md); now shaping the STTR Aim 1 hybrid-definition question), **literature/market notes drafted** ([LITERATURE_AND_MARKET_NOTES.md](LITERATURE_AND_MARKET_NOTES.md)), **hybrid-definition first-pass test complete** ([HYBRID_DEFINITION_FIRST_PASS.md](HYBRID_DEFINITION_FIRST_PASS.md): single exploratory run, does not clear the benchmark, a real open question for the funded Aim 1), **bootstrap CIs + paired significance testing on the 3-definition comparison complete** ([DEFINITION_BOOTSTRAP_COMPARISON.md](DEFINITION_BOOTSTRAP_COMPARISON.md): hybrid is confidently worse than both single definitions; fixed-vs-relative remains statistically indistinguishable, an open question), **calibrator transfer test complete** ([CALIBRATOR_TRANSFER_TEST.md](CALIBRATOR_TRANSFER_TEST.md): the LBNL-fit Platt calibrator does not transfer cleanly to the 100-home cohort; calibration appears deployment-specific), **regime-stratified feasibility check complete** ([REGIME_STRATIFIED_CHECK.md](REGIME_STRATIFIED_CHECK.md): mixed result, no benefit from regime-specific training in the majority low-baseline homes, a modest but data-starved-and-uncertain edge in the small high-baseline group), **technical-approach note for Aims 1 and 2 drafted** ([TECHNICAL_APPROACH_NOTE.md](TECHNICAL_APPROACH_NOTE.md)), **regime gap replication tested in the 100-home cohort** ([REGIME_REPLICATION_100HOME.md](REGIME_REPLICATION_100HOME.md): **does not replicate directionally**. Low-baseline detection is consistent (45.0% vs 45.6%), but high-baseline flips from the weak point in LBNL (13.6%) to the stronger one here (68.0%), at the cost of a false-alarm rate above target (a different failure mode, not a confirmed repeat of the same one)), **recalibration data-requirement simulation complete** ([RECALIBRATION_DATA_REQUIREMENT.md](RECALIBRATION_DATA_REQUIREMENT.md): no clean monotonic answer in this first pass; slope trends toward ideal around 60–180 days/~30–130 positive windows, but a likely test-window-shrinkage artifact prevents a confident number).
...
**Empirical rounds are now paused** (PI's call, after 3 rounds of honestly-reported findings) in favor of writing deliverables: [RESEARCH_STRATEGY_DRAFT.md](RESEARCH_STRATEGY_DRAFT.md) (full Aim 1 & Aim 2 draft, expanding `TECHNICAL_APPROACH_NOTE.md`) and [INNOVATION_SECTION_DRAFT.md](INNOVATION_SECTION_DRAFT.md) (positions the event-level reframing, not the underlying model, as the real novelty, plus the IQAir room-level-vs-city-level distinction).
...
**LBNL California Homes Ventilation & IAQ Dataset** (Dryad, DOI [10.7941/D1ZS7X](https://datadryad.org/stash/dataset/doi:10.7941/D1ZS7X)): 70 detached CA homes (built 2011–2017), each monitored ~1 week, with simultaneous indoor + outdoor PM2.5, indoor CO2, formaldehyde, NO2/NOx, plus mechanical-ventilation airflow/leakage diagnostics. Open download, no access gate. It's the only source found that pairs indoor+outdoor PM2.5 with ventilation context across a large enough building count (n=70) for building-held-out cross-validation.

- Sampling frequency is not yet confirmed from the dataset files; needs direct inspection to confirm it supports a 30–60 min horizon (need ≥3x the horizon in lead samples). If hourly, horizon shifts to 60–120 min.
- The windows-closed protocol, 1 week/home, CA-only, limits generalization claims about natural ventilation and other climates.
...
Proposed definition: indoor PM2.5 ≥ 35 µg/m³ (EPA 24-hr NAAQS level, standard acute-elevation proxy in episodic-exposure literature), sustained for ≥15 consecutive minutes above threshold (≥2 consecutive readings if coarser sampling forces it), with a prediction horizon of 30–60 min (exact value contingent on confirming the primary dataset's sampling interval), framed as binary classification: "episode onset within horizon window" vs. persistence/no-onset.
...
- Indoor/outdoor infiltration modeling is supported by the LBNL dataset (it has both signals).
- CO2-based ventilation-event prediction has no dataset that combines multi-building scale, CO2, and explicit HVAC/window state. Best partial fits: HPDmobile (6 homes, occupancy but no HVAC/window state) or Aalborg University's Danish office dataset (rich HVAC/window state, single building only). Treated as exploratory/secondary per PI direction.
...
The remaining 7 papers surveyed are figure-embedded or "available upon request" only. They're relevant endpoint panels for the STTR narrative, but not raw downloadable data today.

### Recommendation

Data support a feasibility test. Proposed lock-in: LBNL dataset as primary corpus (pending sampling-frequency confirmation), 100-homes dataset as external generalization test, 35 µg/m³ / 15-min / 30–60-min episode definition, building-held-out cross-validation. Proceed to Step 2 baselines once confirmed by PI.
